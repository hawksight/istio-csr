...
We use the `default` namespace for simplicity, so let's label the namesapce for istio injection:

```shell
kubectl label namespace $NAMESPACE istio-injection=enabled --overwrite
```

In a seperate terminal you should now follow the logs for `cert-manager`:
...
In another seperate terminal, lets watch the `istio-system` namesapce for `certificaterequests`:
